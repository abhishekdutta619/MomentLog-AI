...
function MomentsContent() {
  ...
  const moodParam = searchParams.get("mood"); // "1"-"5" | null, from the dashboard's mood buttons

  const parsedMood = moodParam ? Number(moodParam) : null;
  const initialMood = parsedMood && parsedMood >= 1 && parsedMood <= 5 ? parsedMood : null;
